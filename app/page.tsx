...
function jalali(value: string) {
  ...
  const y=gy-1600
  let days=365*y+Math.floor((y+3)/4)-Math.floor((y+99)/100)+Math.floor((y+399)/400)+gd-1+gdm[gm-1]
  if(gm>2&&((gy%4===0&&gy%100!==0)||gy%400===0))days++
  let d=days-79; let jy=979+33*Math.floor(d/12053); d%=12053
  jy+=4*Math.floor(d/1461); d%=1461
  if(d>365){jy+=Math.floor((d-1)/365);d=(d-1)%365}
  const jm=d<186?1+Math.floor(d/31):7+Math.floor((d-186)/30),jd=1+(d<186?d%31:(d-186)%30)
...
export default function Home(){
 ...
 async function load(r=role){setLoading(true);setError('');try{const j=await call('dashboard');setData(j)}catch(e:any){setError(e.message||'خطا در دریافت اطلاعات')}finally{setLoading(false)}}
