...
async function call(action: string, body: Record<string, any> = {}, token = '') {
...
export default function Home(){
 const [username,setUsername]=useState(''),[password,setPassword]=useState(''),[role,setRole]=useState<'manager'|'resident'>('manager'),[token,setToken]=useState(''),[data,setData]=useState<any>({}),[loading,setLoading]=useState(false),[error,setError]=useState(''),[register,setRegister]=useState(false),[building,setBuilding]=useState(''),[address,setAddress]=useState(''),[confirm,setConfirm]=useState('')
 useEffect(()=>{const t=localStorage.getItem('sy_session'),r=localStorage.getItem('sy_role') as any;if(t&&r){setToken(t);setRole(r);load(t,r)}},[])
 async function load(t=token,r=role){setLoading(true);setError('');try{const j=await call(r==='manager'?'manager_dashboard':'resident_dashboard',{},t);setData(j)}catch(e:any){setError(e.message||'خطا در دریافت اطلاعات')}finally{setLoading(false)}}
 async function login(){setLoading(true);setError('');try{const j=await call(role==='manager'?'login_manager':'login_resident',{username:username.trim(),password});const t=j.access_token||j.session;if(!t)throw new Error('ورود ناموفق بود');localStorage.setItem('sy_session',t);localStorage.setItem('sy_role',role);setToken(t);await load(t,role)}catch(e:any){setError(e.message||'ورود ناموفق بود')}finally{setLoading(false)}}
 ...
 function logout(){localStorage.removeItem('sy_session');localStorage.removeItem('sy_role');setToken('');setData({})}
