...
public final class JalaliDate {
    ...
    private static int[] toGregorian(int jy,int jm,int jd){jy+=1595;int days=-355668+365*jy+(jy/33)*8+((jy%33+3)/4)+jd+(jm<=7?31*(jm-1):30*(jm-7)+186);int gy=400*(days/146097);days%=146097;if(days>36524){days--;gy+=100*(days/36524);days%=36524;if(days>=365)days++;}gy+=4*(days/1461);days%=1461;if(days>365){gy+=(days-1)/365;days=(days-1)%365;}boolean leap=(gy%4==0&&gy%100!=0)||gy%400==0;int[] md={31,28+(leap?1:0),31,30,31,30,31,31,30,31,30,31};int gm=1;while(gm<=12&&days>=md[gm-1]){days-=md[gm-1];gm++;}return new int[]{gy,gm,days+1};}
